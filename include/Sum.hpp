...
template<int N>
struct Negative<N, false> {
    using res = typename convertIntDigit<10 + N>::D;
};
    
template<int N>
struct Negative<N, true> {
    using res = typename convertIntDigit<N>::D;
};

template <typename D1, typename D2>
struct SubtractDigits {
    static constexpr int diff = D1::val - D2::val;
    using type = typename Negative<diff, diff >= 0>::res;
